...
import { glossary, projectFiles, stages, taskTypeGuide, type Task } from "@/lib/guide";
import { ArrowIcon, BookIcon, CheckIcon, SparkleIcon } from "./icons";
import { CodeBlock } from "./code-block";

const storageKey = "cityride-progress-v2";
  ...
  return <div className={`app-shell ${sidebarCollapsed?"sidebar-is-collapsed":""}`}>
    ...
    <main className="main-area">
      ...
      <section className="content-wrap"><div className="section-heading"><div><span className="eyebrow">{learners[learner].name}&apos;s to-do list</span><h2>One small win at a time.</h2></div><span className="stage-count">{stageDone} of {stageTasks.length} done</span></div>
        <div className="task-list">{stageTasks.map((task,index)=>{const isDone=completed[learner].includes(task.id);return <article className={`task-card ${isDone?"is-done":""} kind-${task.kind}`} key={task.id}><button className="check-button" onClick={()=>toggle(task.id)} aria-label={`${isDone?"Mark incomplete":"Mark complete"}: ${task.title}`} aria-pressed={isDone}>{isDone?<CheckIcon/>:<span>{index+1}</span>}</button><div className="task-body"><div className="task-meta"><span className={`task-kind ${task.kind}`}>{task.kind==="learn"?"Learn":task.kind==="follow"?"Follow along":"Challenge"}</span><span className={`owner ${task.assignee}`}>{task.assignee==="both"?"Together":learners[task.assignee].name}</span>{task.file?<code>{task.file}</code>:null}</div><h3>{task.title}</h3><p>{task.description}</p><div className="action-list"><ol>{task.steps.map(item=><li key={item}>{item}</li>)}</ol></div>{task.solution?<details className="solution"><summary>Show one answer</summary><ol className="solution-list">{task.solution.map(item=><li key={item}>{item}</li>)}</ol></details>:task.code?(task.kind==="challenge"?<details className="solution"><summary>Show one answer</summary><CodeBlock code={task.code}/></details>:<CodeBlock code={task.code}/>):null}{task.links?<div className="task-references">{task.links.map(link=><a key={link.url} href={link.url} target="_blank" rel="noreferrer">{link.label}<ArrowIcon/></a>)}</div>:null}</div></article>;})}</div>
